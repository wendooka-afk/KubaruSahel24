...
const ArticleCard: React.FC<ArticleCardProps> = ({ article, variant = 'vertical' }) => {
  ...
  if (variant === 'compact') {
    const slug = article.seo?.slug || slugify(article.title);
    return (
      <Link to={`/${slugify(article.category)}/${slug}`} className="group flex gap-4 items-start border-b border-gray-100 pb-4 last:border-0 hover:bg-gray-50/80 p-2 rounded-lg transition-all duration-300">
  ...
  if (variant === 'horizontal') {
    const slug = article.seo?.slug || slugify(article.title);
    return (
      <Link to={`/${slugify(article.category)}/${slug}`} className="group grid grid-cols-1 md:grid-cols-12 gap-6 mb-8 items-center bg-white p-4 rounded-2xl border border-transparent hover:border-gray-100 hover:shadow-xl transition-all duration-300">
        ...
        <div className="md:col-span-7 flex flex-col justify-center">
          ...
          <div className="text-xs text-gray-400 mt-auto font-medium">{new Date(article.publishedAt).toLocaleDateString(t('header.date'), { day: 'numeric', month: 'long', year: 'numeric' })}</div>
        </div>
      </Link>
    );
  }
  ...
  const slug = article.seo?.slug || slugify(article.title);
  return (
    <Link to={`/${slugify(article.category)}/${slug}`} className="group flex flex-col h-full bg-white rounded-2xl border border-transparent hover:border-gray-50 hover:shadow-xl transition-all duration-300 p-2">
      ...
      <div className="px-2 pb-4 flex flex-col flex-grow">
        ...
        <div className="flex items-center justify-between text-[11px] text-gray-400 mt-auto border-t border-gray-100 pt-3 font-medium">
          <span className="flex items-center gap-1"><Clock size={12} /> {new Date(article.publishedAt).toLocaleDateString(t('header.date'))}</span>

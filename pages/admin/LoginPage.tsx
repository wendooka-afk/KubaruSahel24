...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-primary flex items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      >
        <ArrowLeft size={20} /> {t('admin.login.backToSite')}
      </button>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-12 w-full max-w-md relative z-10 animate-fade-in border border-white/10">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400 font-bold text-xs uppercase tracking-widest">{t('admin.login.title')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('admin.login.emailLabel')}</label>
            <input
              type="email"
              value={email}
          ...
          <div className="space-y-2">
            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('admin.login.passwordLabel')}</label>
            <input
              type="password"
              value={password}
        ...
        <div className="mt-10 p-5 bg-gray-50 rounded-2xl border border-gray-100">
          <p className="text-[10px] font-bold text-gray-400 uppercase mb-2 tracking-widest text-center">{t('admin.login.demo')}</p>
